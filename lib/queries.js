...
module.exports = {
  ...
  setGym: function(inGym, col, val) {
    return knex.raw(`UPDATE pokemon SET in_gym='${inGym}' WHERE ${col}=${val}`);
  },
  delete: function(id) {
    return knex.raw(`DELETE from pokemon WHERE id=${id}`);
  },
  addCp: function(id, cp) {
    return knex.raw(`UPDATE pokemon SET cp=(20 + ${cp}) WHERE id=${id}`);
  }
}
